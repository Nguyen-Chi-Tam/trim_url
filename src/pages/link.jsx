...
const Link = () => {
  ...
  const handleCreateQr = async () => {
    ...
    try {
      setCreatingQr(true);
      const shortFull = `https://trimurlz.me/${url.short_url}`;
      const blob = await generateQrBlob(shortFull);
      const file = new File([blob], 'qrcode.png', { type: 'image/png' });
      await attachQrCode({ id: url.id, short_url: url.short_url }, file);
      await fn(); // reload url
    } catch (e) {
  ...
  const handleDeleteQr = async () => {
    if (!url) return;
    try {
      setDeletingQrState(true);
      await deleteQrCode({ id: url.id });
      await fn();
    } catch (e) {
      console.error('Error deleting QR:', e);
    } finally {
      setDeletingQrState(false);
    }
  }
            ...
            {isEditing ? (
              ...
              <>
                ...
                {!url?.qr_code ? (
                  ...
                  <Button variant="ghost" onClick={handleDeleteQr} disabled={deletingQrState}>
